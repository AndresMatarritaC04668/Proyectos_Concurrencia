...
void generar_Resultado(simuladorInfo_t * simuladorInfo) {
    ...
    // segundos totales
    double segundos = simuladorInfo->estadok * simuladorInfo->deltaT;
    ...
    if (!lamina) {
      ...
      for (int fila = 0; simuladorPlacas->filas < fila; ++fila) {
       for (int columna = 0; simuladorPlacas->columnas < columna; ++columna) {
        lamina.write(reinterpret_cast<char*>(&simuladorPlacas->
            placa[fila][columna]), sizeof(simuladorPlacas->
                placa[fila][columna]));
       }
      }
      lamina.close();
...
double simuladorPlacas_sumarCruz(simuladorPlacas_t* simulador,
    ...
    resultadoultado = resultadoultado + simuladorPlacas_getCelda(simulador,
      fila, columna+1) + simuladorPlacas_getCelda(simulador, fila, columna - 1);
    resultadoultado = resultadoultado + simuladorPlacas_getCelda(simulador,
        fila+1, columna) + simuladorPlacas_getCelda(simulador, fila-1, columna);
    //  se resultadota la energia perdida y distribuida a las otras celdas
    resultadoultado = resultadoultado - 4*actual;
    //  multiplicamos por el tiempo y difusividad termica entre
    //  alto de la celda al cuadrado
    double tiDiAl = ((simulador->deltaT * simulador->disTermA)/
            (simulador->altoH*simulador->altoH));
